...
async function accessToken(accountId: string) {
  const account = await db.googleAccount.findUniqueOrThrow({
    where: { id: accountId },
  });
...
export async function assertCalendarAvailability(
  accountId: string,
  startAt: Date,
  endAt: Date,
) {
  ...
  if (busy.length)
    throw new Error("Organizer is unavailable at the requested time");
}
...
function eventPayload(
  ...
  return {
    ...
    conferenceData: {
      createRequest: {
        requestId: `flowos-meeting-${meeting.id}`,
...
export async function createCalendarMeeting(
  accountId: string,
  meeting: Parameters<typeof eventPayload>[0],
  attendees: Array<{ email: string }>,
) {
  const account = await db.googleAccount.findUniqueOrThrow({
    where: { id: accountId },
  ...
  return googleFetch(
    accountId,
    `/calendars/${encodeURIComponent(account.calendarId)}/events?conferenceDataVersion=1&sendUpdates=all`,
    {
      method: "POST",
      body: JSON.stringify(eventPayload(meeting, attendees)),
    },
...
export async function findCalendarMeeting(
  accountId: string,
  meetingId: string,
) {
  const account = await db.googleAccount.findUniqueOrThrow({
  ...
  const query = new URLSearchParams({
    privateExtendedProperty: `flowosMeetingId=${meetingId}`,
    singleEvents: "true",
    maxResults: "1",
  });
...
export async function updateCalendarMeeting(
  ...
  return googleFetch(
    accountId,
    `/calendars/${encodeURIComponent(account.calendarId)}/events/${encodeURIComponent(eventId)}?conferenceDataVersion=1&sendUpdates=all`,
    {
      method: "PATCH",
      body: JSON.stringify(eventPayload(meeting, attendees)),
    },
...
export async function cancelCalendarMeeting(
  ...
  await googleFetch(
    accountId,
    `/calendars/${encodeURIComponent(account.calendarId)}/events/${encodeURIComponent(eventId)}?sendUpdates=all`,
    { method: "DELETE" },
  );
}
